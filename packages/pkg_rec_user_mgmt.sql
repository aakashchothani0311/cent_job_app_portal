CREATE OR REPLACE PROCEDURE CREATE_REC_USER (
...
BEGIN
        ...
        SELECT
        SUM(CASE WHEN LOWER(USERNAME) = LOWER(PI_USERNAME) THEN 1 ELSE 0 END) AS USERNAME_COUNT,
        SUM(CASE WHEN LOWER(EMAIL) = LOWER(PI_EMAIL) THEN 1 ELSE 0 END) AS EMAIL_COUNT
        INTO
            V_USERNAME, V_EMAIL
        FROM USERS;
    ...
    --    END IF;
        ...
        -- Insert New User
            ...
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('USER CREATED SUCCESSFULLY. USER_ID:' || V_NEW_USER_ID));
    COMMIT;
    
EXCEPTION
    -- Handle exceptions
    WHEN NULL_FNAME_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: FIRSTNAME cannot be empty.'));
    WHEN NULL_LNAME_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: LASTNAME cannot be empty.'));
    WHEN NULL_USERNAME_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: USERNAME cannot be empty. '));
    WHEN DUP_USERNAME THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: USERNAME already exists. '));
    WHEN NULL_EMAIL_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: EMAIL cannot be empty. '));
    WHEN DUP_EMAIL THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: EMAIL already exists. '));
    WHEN NULL_PASSWORD_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error creating PROFILE: PASSWORD cannot be empty or less than 8 characters. '));
...
CREATE OR REPLACE PROCEDURE UPDATE_REC_USER(
...
BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('USERNAME: "' || PI_USERNAME || '" DOES NOT EXIST.'));
    END;
    ...
    IF LENGTH(TRIM(PI_PASSWORD)) IS NULL  OR LENGTH(TRIM(PI_PASSWORD)) < 8 THEN
        RAISE NULL_PASSWORD_EXEC;
    END IF;
    ...
    IF SQL%ROWCOUNT > 0 THEN
        UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('USER "' || PI_USERNAME || '" UPDATED SUCCESSFULLY.'));
    ELSE
    ...
    EXCEPTION
        ...
        WHEN NULL_FNAME_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error updating FIRSTNAME: FIRSTNAME cannot be empty.'));
        WHEN NULL_LNAME_EXEC THEN
            UTIL_PKG.ADD_NEW_LINE(UTIL_PKG.ADD_TAB('Error updating LASTNAME: LASTNAME cannot be empty.'));
        WHEN NULL_PASSWORD_EXEC THEN
