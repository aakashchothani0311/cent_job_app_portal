...
BEGIN
    ...
    SELECT COUNT(CANDIDATE_ID) INTO V_COUNT
    FROM CANDIDATES
    ...
    IF PI_VETERAN IS NOT NULL AND TRIM(PI_VETERAN) NOT IN 
       ('do not wish to disclose','not a veteran','I am a protected veteran') THEN
        RAISE INVALID_VETERAN_EXEC;
    END IF;

    IF PI_DISABILITY IS NOT NULL AND TRIM(PI_DISABILITY) NOT IN
       ('do not wish to disclose','no, i do not have a disability','yes, I have a disability') THEN
