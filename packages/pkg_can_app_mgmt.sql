...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No matching application found for Candidate ID: ' || PI_CANID || ' and Requisition ID: ' || PI_REQID);
    ELSE
        DBMS_OUTPUT.PUT_LINE('Application status updated successfully for Candidate ID: ' || PI_CANID || ' and Requisition ID: ' || PI_REQID  || 'and Status as: ' || PI_STATUS);
        COMMIT;
    END IF;
...
EXCEPTION
    ...
    WHEN INVALID_CANDIDATE_EXEC THEN
        DBMS_OUTPUT.PUT_LINE('Error: Candidate ID ' || PI_CANID || ' does not exist.');
    WHEN INVALID_REQUISITION_EXEC THEN
        DBMS_OUTPUT.PUT_LINE('Error: Requisition ID ' || PI_REQID || ' does not exist.');
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error updating application status: ' || SQLERRM);
        ROLLBACK;
