...
-- PROCEDURE FOR Creating Candidate User
    ...
    EXCEPTION
        WHEN NULL_FNAME THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: First name cannot be empty.');
        WHEN NULL_LNAME THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Last name cannot be empty.');
        WHEN NULL_EMAIL THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Email cannot be empty.');
        WHEN NULL_USERNAME THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Username cannot be empty.');
        WHEN NULL_PASSWORD THEN 
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Password cannot be empty.');
        WHEN NULL_ST1 THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Street 1 cannot be empty.');
        WHEN NULL_CITY THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: City cannot be empty.');
        WHEN NULL_STATE THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: State cannot be empty.');
        WHEN NULL_COUNTRY THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Country cannot be empty.');
        WHEN INVALID_GENDER THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Gender must be "male" or "female".');
        WHEN INVALID_AGE THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: Age must be greater than 16.');
        WHEN INVALID_VETERAN THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: 
        ...
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error creating candidate user profile: ' || SQLERRM);
            ROLLBACK;
...
-- PROCEDURE FOR Deactivating Candidate account
    ...
    EXCEPTION
        WHEN NULL_CANDIDATE_ID THEN
            DBMS_OUTPUT.PUT_LINE('Error deactivating candidate account: Candidate ID cannot be empty.');
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error deactivating candidate account: ' || SQLERRM);
            ROLLBACK;
...
-- FUNCTION FOR Checking if a Candidate is active in our database
    ...
    EXCEPTION
        WHEN NULL_CANDIDATE_ID THEN
            DBMS_OUTPUT.PUT_LINE('Error in checking candidate account status: Candidate ID cannot be empty.');
            RETURN 0;
            
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('Error in checking candidate account status: Candidate not found.');
            RETURN 0;
            
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('Error in checking candidate account status:: ' || SQLERRM);
            RETURN 0;
